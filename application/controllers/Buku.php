...
class Buku extends MY_Controller {
    ...
    {
        $data['buku']      = $this->db->join('kategori_buku', 'id_kategori_buku = kategori_buku')->order_by('buku.kode_buku desc')->get('buku');
        ...
        if(isset($_POST['save'])) {
            ...
            if($this->form_validation->run()==true){
                ...
                if($cek->num_rows() > 0){
                    ...
                    if($_FILES['ebook']){
                        $config['upload_path']   = './assets/pdf/ebook/';
                        ...
                        $ebook = $this->upload->data();
                    }
                     //apabila ada gambar yg diupload
                    if (!empty($_FILES['userfile']['name']) && !empty($_FILES['ebook']['name'])){
                        $save  = array(
                            'kode_buku'   => $this->input->post('kode_buku'),
            ...
            if(!empty($_FILES['userfile']['name']) || !empty($_FILES['ebook']['name'])) {
                ...
                if($this->form_validation->run()==true){
                    ...
                    if (!empty($_FILES['ebook']['name'])){
                        ...
                        //hapus gambar yg ada diserver
                        unlink('assets/pdf/ebook/'.$g['ebook']);
    ...
    public function delete()
    {
        ...
        //hapus gambar yg ada diserver
        unlink('assets/img/buku/'.$g['image']);
        unlink('assets/pdf/ebook/'.$g['image']);
